...
const { RegisterService, LoginService, ForgetPasswordService, ConfirmPasswordService } = require("../../services/AuthService");
...
const ConfirmPassword =(event)=>{
  ...
  const requestBody = JSON.parse(event.body);
  const { email,password,code } = requestBody;
  if (!requestBody || Object.keys(requestBody).length === 0) {
    return responseBuilder(STATUS_CODE.BAD_REQUEST, ERROR_MESSAGE.EMPTY_BODY);
  }
  // const validateResult = validateLoginAttributes({
  ...
  return ConfirmPasswordService(password,code,email)
}

module.exports = { Register, Login, ForgotPassword, ConfirmPassword };
